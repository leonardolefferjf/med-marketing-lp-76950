...
const testimonialImages = [provaSocial1, provaSocial2, provaSocial3, provaSocial4, provaSocial5, provaSocial6, provaSocial7, provaSocial8, provaSocial9, provaSocial10];
        ...
        <div className="bg-gradient-to-br from-gray-100 to-gray-50 border-2 border-gray-300 rounded-2xl p-6 md:p-10 shadow-xl">
          ...
          <div className="relative">
            ...
            <div className="bg-white rounded-xl p-4 shadow-2xl max-w-3xl mx-auto mb-8">
              <img src={testimonialImages[currentSlide]} alt={`Depoimento de médico ${currentSlide + 1}`} className="w-full h-auto rounded-lg" loading="lazy" />
            </div>

            {/* Navigation */}
            <div className="flex justify-center items-center gap-4 mb-4">
              <button onClick={prevSlide} className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white p-3 rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all" aria-label="Depoimento anterior">
                <ChevronLeft className="w-6 h-6" />
              </button>
              
              <div className="flex gap-2">
                {testimonialImages.map((_, index) => <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-all ${index === currentSlide ? 'bg-green-600 w-8' : 'bg-gray-400 hover:bg-gray-500'}`} aria-label={`Ir para depoimento ${index + 1}`} />)}
              </div>
              
              <button onClick={nextSlide} className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white p-3 rounded-full shadow-lg hover:shadow-xl transform hover:scale-105 transition-all" aria-label="Próximo depoimento">
